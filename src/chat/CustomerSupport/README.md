...
products:
- dotnet
name: Customer support ticket summarizer
urlFragment: customer-support-tickets
---
...
- *ManualIngestor.cs* - Ingestion service that extracts data from product manual PDF files, chunks text into smaller segments, generates embeddings, and saves them to a JSON file.
- *ProductManualService.cs* - Storage service that uses an `IVectorStore` to save and search product manual embeddings.
- *TicketSummarizer* - AI service that uses an AI model to generate summaries of customer support tickets.
